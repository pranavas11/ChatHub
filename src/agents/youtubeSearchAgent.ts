...
const basicYoutubeSearchResponsePrompt = `
    You are ChatHub, an AI model who is expert at searching the web and answering user's queries. You are set on focus mode 'Youtube', this means you will be searching for videos on the web using Youtube and providing information based on the video's transcript.
...
const createBasicYoutubeSearchRetrieverChain = (llm: BaseChatModel) => {
  const promptTemplate = PromptTemplate.fromTemplate(basicYoutubeSearchRetrieverPrompt);

  return RunnableLambda.from(async (input: { query: string; chat_history: string }) => {
    try {
      ...
      // Step 3: Parse the response to string
      const parsedResponse = await strParser.invoke(llmResponse);

      // Step 4: Search YouTube if needed
      if (parsedResponse === "not_needed") {
        return { query: "", docs: [] };
      }
      ...
      return { query: parsedResponse, docs: documents };
    } catch (error) {
      console.error("Error in YouTube search retriever chain:", error);
      return { query: "", docs: [] };
    }
  });
};
...
const createBasicYoutubeSearchAnsweringChain = (
  ...
  // Create the context retrieval chain
  const contextChain = RunnableLambda.from(async (input: BasicChainInput) => {
    try {
      // Get search results
      const searchResults = await basicYoutubeSearchRetrieverChain.invoke({
        query: input.query,
        chat_history: formatChatHistoryAsString(input.chat_history),
      });

      // Rerank documents
      const rerankedDocs = await rerankDocs(searchResults);

      // Process documents to context string
      const context = await processDocs(rerankedDocs);
  ...
  return RunnableLambda.from(async (input: BasicChainInput) => {
    try {
      // Step 1: Get context
      const context = await contextChain.invoke(input);

      // Step 2: Format the chat prompt
      ...
      const llmResponse = await llm.invoke(formattedMessages);
      ...
      console.error("Error in YouTube search answering chain:", error);
      throw error;
    }
  }).withConfig({
    runName: "FinalResponseGenerator",
