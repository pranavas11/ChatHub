...
const basicAcademicSearchResponsePrompt = `
    You are ChatHub, an AI model who is expert at searching the web and answering user's queries. You are set on focus mode 'Acadedemic', this means you will be searching for academic papers and articles on the web.
...
const createBasicAcademicSearchAnsweringChain = (
  ...
  const contextChain = RunnableLambda.from(async (input: BasicChainInput) => {
    try {
      // Get search results
      const searchResults = await basicAcademicSearchRetrieverChain.invoke({
        query: input.query,
        chat_history: formatChatHistoryAsString(input.chat_history),
      });
  ...
  return RunnableLambda.from(async (input: BasicChainInput) => {
    try {
      ...
      console.error("Error in academic search answering chain:", error);
      throw error;
    }
  }).withConfig({
    runName: "FinalResponseGenerator",
...
const createBasicAcademicSearchRetrieverChain = (llm: BaseChatModel) => {
  return RunnableLambda.from(async (input: { query: string; chat_history: string }) => {
    try {
      ...
      const promptTemplate = PromptTemplate.fromTemplate(basicAcademicSearchRetrieverPrompt);
      ...
      console.error("Error in academic search retriever chain:", error);
      return { query: "", docs: [] };
    }
  });
};
