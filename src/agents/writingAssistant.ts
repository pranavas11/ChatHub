...
You are ChatHub, an AI model who is expert at searching the web and answering user's queries. You are currently set on focus mode 'Writing Assistant', this means you will be helping the user write a response to a given query. 
...
const createWritingAssistantChain = (llm: BaseChatModel) => {
  ...
  return RunnableLambda.from(async (input: BasicChainInput) => {
    try {
      // Step 1: Format the prompt with chat history and query
      ...
      console.error("Error in writing assistant chain:", error);
      throw error;
    }
  }).withConfig({
    runName: "FinalResponseGenerator",
