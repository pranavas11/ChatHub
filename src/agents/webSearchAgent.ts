...
const basicWebSearchResponsePrompt = `
    You are Jupyter, an AI model who is expert at searching the web and answering user's queries.
...
const createBasicWebSearchRetrieverChain = (llm: BaseChatModel) => {
  const promptTemplate = PromptTemplate.fromTemplate(basicSearchRetrieverPrompt);
  ...
  return RunnableLambda.from(async (input: { query: string; chat_history: string }) => {
    try {
      ...
      console.error("Error in web search retriever chain:", error);
      return { query: "", docs: [] };
    }
  });
};
...
const createBasicWebSearchAnsweringChain = (
  ...
  const contextChain = RunnableLambda.from(async (input: BasicChainInput) => {
    try {
      // Get search results
      const searchResults = await basicWebSearchRetrieverChain.invoke({
        query: input.query,
        chat_history: formatChatHistoryAsString(input.chat_history),
      });
  ...
  return RunnableLambda.from(async (input: BasicChainInput) => {
    try {
      ...
      console.error("Error in web search answering chain:", error);
      throw error;
    }
  }).withConfig({
    runName: "FinalResponseGenerator",
