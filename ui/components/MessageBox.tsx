...
const MessageBox = ({
  ...
  useEffect(() => {
    if (message.role === "assistant" && message?.sources && message.sources.length > 0) {
      ...
      message.suggestions = [
        "tell me about the products at apple",
        "tell me about someone's personal journey",
      ];
                    ...
                    <div className="flex flex-col space-y-3">
                      {message.suggestions.map((suggestion, i) => (
                        <div key={i} className="flex flex-col space-y-3 text-sm">
                          <div className="h-px w-full bg-[#1C1C1C]" />
                          <div onClick={() => sendMessage(suggestion)} className="cursor-pointer flex flex-row justify-between font-medium space-x-2 items-center">
